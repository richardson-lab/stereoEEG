...
def generate_ashifts(A, Nperm, tperm, sample_rate):
    interval_size = cp.diff(cp.array(tperm))[0]
    random_tshifts = tperm[0] + interval_size * cp.random.rand(Nperm)
    random_shifts = cp.rint(random_tshifts * sample_rate).astype(int)
    N = A.size
    permuted_data = cp.empty((Nperm, N), dtype=A.dtype)
    for i, shift in enumerate(random_shifts):
        permuted_data[i, :] = cp.roll(A, shift)
