...
% parameters
ddir = '/Users/drew/Box/sEEG/HUP251_RID923'; % data directory
sess = 'HUP251_RID923_Induction.mat'; % session data
...
dtOnN = diff(tOnN);
ishort = find(dtOnN < 1); % remove spurious natus tone onsets (too short to be possible)
if ~isempty(ishort)
    tOnN(ishort+1) = [];
    dtOnN = diff(tOnN);
end
...
[~,imx] = max(r);
lmx = lags(imx);
if lmx>=0 % synchronized tone-onset indices
    isyncB = lmx+1:length(dtOnB); 
    isyncN = 1:min(length(dtOnN),length(isyncB));
else
    isyncN = abs(lmx)+1:length(dtOnB); 
    isyncB = 1:min(length(dtOnB),length(isyncN));
end
